<!-- The charts below is only available in the 'bar' package -->
	...
	$sql = "SELECT p11, pDataTime FROM $db_table ORDER BY pIndex";

	$result = $connection->query($sql);  

	# This while - loop formats and put all the retrieved data into ['pDataTime', 'p11'] way.
		while ($row = $result->fetch_assoc()) {
			$timestamp_rest = $row["pDataTime"];
			echo "['".$timestamp_rest."',".$row['p11']."],";
			}
	?>
	]);
	...
	<div id="barchart_values11" style="width: 300px; min-height: 2000px;"></div>