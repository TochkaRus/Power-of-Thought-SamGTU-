<!-- The charts below is only available in the 'bar' package -->
	...
	$sql = "SELECT p8, pDataTime FROM $db_table ORDER BY pIndex";

	$result = $connection->query($sql);  

	# This while - loop formats and put all the retrieved data into ['pDataTime', 'p8'] way.
		while ($row = $result->fetch_assoc()) {
			$timestamp_rest = $row["pDataTime"];
			echo "['".$timestamp_rest."',".$row['p8']."],";
			}
	?>
	]);
	...
	// Bar chart
	var chart = new google.visualization.BarChart(document.getElementById('barchart_values8'));
	chart.draw(data, bar_options);
	} // End bracket from drawBar


	</script>
	<div id="barchart_values8" style="width: 200px; min-height: 2000px;"></div>