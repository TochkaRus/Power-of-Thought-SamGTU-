<!-- The charts below is only available in the 'bar' package -->
	...
	$sql = "SELECT p13, pDataTime FROM $db_table ORDER BY pIndex";

	$result = $connection->query($sql);  

	# This while - loop formats and put all the retrieved data into ['pDataTime', 'p13'] way.
		while ($row = $result->fetch_assoc()) {
			$timestamp_rest = $row["pDataTime"];
			echo "['".$timestamp_rest."',".$row['p13']."],";
			}
	?>
	]);
	...
	var bar_options = {
			...
			colors: ['#FF00FF'],    // any HTML string color ('red', '#cc00cc')
	...
	// Bar chart
	var chart = new google.visualization.BarChart(document.getElementById('barchart_values13'));
	chart.draw(data, bar_options);
	} // End bracket from drawBar


	</script>
	<div id="barchart_values13" style="width: 200px; min-height: 2000px;"></div>